...
- openai SDK pin `~=3.7` (last checked 3.7.0 on 2026-09-03; `prompt_cache_retention` is still a typed `responses.create` parameter there). `prompt_cache_retention` is deprecated in the SDK in favour of `prompt_cache_options.ttl` (GPT-5.6+ only; `30m` is the only ttl), but the API still accepts `24h` on gpt-5.6 (probed 2026-08-28) and it stays the right knob for older models, so `PROMPT_CACHE_RETENTION` is unchanged — migrate when the SDK or API rejects it.
