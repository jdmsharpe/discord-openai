...
- openai SDK pin `~=3.14` (last checked 3.14.0 on 2026-09-15; `prompt_cache_retention` is still a typed `responses.create` parameter there, and the API reference now marks it deprecated). It is superseded by `prompt_cache_options.ttl` (GPT-5.6+ only; `30m` is the only ttl on gpt-6-astra, probed 2026-09-15), but the API still accepts `24h` on gpt-5.6 (probed 2026-08-28) and on gpt-6-astra with the bot's exact payload (2026-09-15) and it stays the right knob for older models, so `PROMPT_CACHE_RETENTION` is unchanged — migrate when the SDK or API rejects it. 3.9.0 added prompt-cache diagnostics (`prompt_cache_options.comparison_response_id` → top-level `response.prompt_cache_diagnostics`, free, no billing effect) and 3.14.0 normalises API error codes to strings; neither is wired.
