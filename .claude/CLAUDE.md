...
- GPT-5.6 (Sol/Terra/Luna, default `gpt-5.6-sol`) accepts reasoning efforts `none`/`low`/`medium`/`high`/`xhigh`/`max` but **rejects `minimal`** with a 400 (live-probed 2026-07-13); the effort menu labels annotate both restrictions. Cached input bills at 10% across the **whole gpt-5 family** via per-model `cached_input_per_million` in pricing.yaml — the 50%-of-input fallback is correct only for the gpt-4o generation, and omitting the field on a gpt-5.x id silently overbills cached input 5×. `test_gpt5_family_declares_ten_percent_cached_rate` pins this (added v1.8.0 after eight ids were found drifted).
