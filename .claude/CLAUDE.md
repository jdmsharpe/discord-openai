...
- Legacy shim: `src/openai_api.py` exists only for import compatibility and emits a `DeprecationWarning`.
...
└── discord_openai/
    ├── __init__.py                  # Re-exports OpenAIAPI
...
- Examples:
  - `discord_openai.cogs.openai.cog.OPENAI_VECTOR_STORE_IDS`
...
- `resolve_selected_tools()` in `discord_openai.cogs.openai.cog` remains the canonical tool-resolution path for chat and research.
