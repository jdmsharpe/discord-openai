<h1 align="center">Discord OpenAI Bot</h1>
...
[![CI](https://github.com/jdmsharpe/discord-openai/actions/workflows/main.yml/badge.svg)](https://github.com/jdmsharpe/discord-openai/actions/workflows/main.yml)
[![Docker Pulls](https://img.shields.io/docker/pulls/jsgreen152/discord-openai-bot?logo=docker&logoColor=white)](https://hub.docker.com/r/jsgreen152/discord-openai-bot)
...
[![Codecov](https://codecov.io/gh/jdmsharpe/discord-openai/branch/main/graph/badge.svg)](https://app.codecov.io/gh/jdmsharpe/discord-openai)
...
- **Cost Embeds:** Each reply ends with one cost line, such as `$0.0120 · 3.4k in (2.1k cached) / 812 out (640 thinking) · 1 search · fast mode · $0.34 today`: the request cost, input tokens (cached ones included and shown in parentheses), output tokens (thinking included and shown in parentheses), tool-call counts, fast mode when used, and your running total for the day. Tool-call fees and cache writes are included in the cost; tools appear only as call counts. GPT-6 and GPT-5.6 cache writes are billed at OpenAI's published 1.25x input surcharge; GPT-5.6 Sol is priced at its promotional rate (guaranteed through 2026-11-21). Image and speech commands show the cost with the item, plus the token counts when the image was billed by tokens, such as `$0.0512 · 48 in / 1.1k out · 1 image · medium · 1024x1024 · $0.34 today`.
...
Create images using GPT Image models (`gpt-image-2.5-sunburst` (default), `gpt-image-2.5-flare`, `gpt-image-2`, `gpt-image-1.5`, `gpt-image-1`, `gpt-image-1-mini`).
